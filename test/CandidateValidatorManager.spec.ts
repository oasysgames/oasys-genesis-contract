...
describe('CandidateValidatorManager', async () => {
  ...
  describe('constructor()', async () => {
    it('should be reverted by `NullAddress`', async () => {
      await expect(factory.deploy(ntoa(0), ntoa(1), ntoa(1))).to.be.revertedWithCustomError(factory, 'NullAddress')
      await expect(factory.deploy(ntoa(1), ntoa(0), ntoa(1))).to.be.revertedWithCustomError(factory, 'NullAddress')
      await expect(factory.deploy(ntoa(1), ntoa(1), ntoa(0))).to.be.revertedWithCustomError(factory, 'NullAddress')
    })
  })
  ...
  describe('afterStateUpdate()', () => {
    it('should be reverted by `UnauthorizedSender`', async () => {
      ...
      await expect(tx).to.be.revertedWithCustomError(factory, 'UnauthorizedSender')
    })
  })
})
